...
use super::addr::{pt_idx, MemFlags, PhysAddr, VirtAddr};
...
impl PageTable {
    ...
    pub fn walk(&mut self, phys: PhysAddr, virt: VirtAddr, mattr: MemFlags, alloc: bool) {
        ...
        error!("Walking page table failed");
    }
...
impl PageTableRoot {
    pub fn new() -> Self {
        ...
        return PageTableRoot {
            root: Mutex::new(
                PageTable::new_with_ptr(root_frame.start_paddr() as usize).brrow_as_mut(),
            ),
            root_frame,
        };
    }
...
pub fn stage2_mmu_init() {
     ...
     */
    let vtcr =
        vtcr_t0sz(20) | vtcr_sl0(2) | vtcr_sh0(0) | vtcr_tg0(0) | VTCR_NSW | VTCR_NSA | vtcr_ps(4);

    info!("Setting vtcr_el2 to 0x{:x}", vtcr);
    write_sysreg!(vtcr_el2, vtcr);

    let mair = (DEVICE_NGNRNE << (8 * DEVICE_NGNRNE_INDEX)) | (NORMAL_WB << (8 * NORMAL_WB_INDEX));
...
pub fn hyper_setup() {
    ...
    info!(
        "Setting hcr_el2 to 0x{:x} and enable stage 2 address translation",
        hcr
    );
